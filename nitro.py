...
TOKEN = 'NTI1MDYxMzA2Mzc0Njg0NzAy.DvxJZA.ieBcfdkNd1OWkMzlvoyBd1YTDgo'
...
async def on_ready():
    ...
    await client.change_presence(game=discord.Game(name='{} servers | n!help | version demo'.format(serverCount), type = 3, status=discord.Status.dnd))
    print('Hello')

@client.event
async def on_member_join(member):
    role = discord.utils.get(member.server.roles, name='Hype')
...
async def help(ctx):
    ...
    embed.set_author(name='Help prefix = n!')
    embed.add_field(name='Common Commands', value='n!help <shows his message> n!invite <link to invite me>', inline=False)
    embed.add_field(name=':tools: Moderation Commands', value='n!kick <user> n!ban <user> n!clear <number of msg> n!userinfo <shows info on a user>', inline=False)
    embed.add_field(name=':smiley: Fun Commands', value='n!ping <returns pong> n!say <what u want it to say> n!flipcoin <random coin>', inline=False)
    embed.add_field(name=':page_facing_up: Info', value='n!info <shows my information> n!serverinfo <servers info>', inline=False)
    embed.add_field(name='Info', value='Created By Goku#0001',inline=False)
...
async def invite(ctx):
	await client.say('Here is an invite link https://discordapp.com/oauth2/authorize?client_id=525061306374684702&permissions=8&scope=bot')
